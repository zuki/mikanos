...
namespace fat {
     ...
     * 単名の拡張子が空なら "<base>" を、空でなければ "<base>.<ext>" をコピー。
     *
     * @param entry ファイル名を得る対象のディレクトリエントリ
     ...
     * @return 次のクラスタ番号（ない場合はkEndOfClusterchain）
     */
    unsigned long NextCluster(unsigned long cluster);

    /** @brief 指定されたディレクトリからパスを探す。
     *
     * @param path パス（大文字小文字は区別しない）
     ...
     */
    unsigned long ExtendCuster(unsigned long eoc_cluster, size_t n);
     ...
     * @return 空きエントリ
     */
    DirectoryEntry *AllocateEntry(unsigned long dir_cluseter);
    ...
    /** @brief 指定した数の空きクラスタからなるチェーンを編集する。
     *
     * @param n クラスタ数
