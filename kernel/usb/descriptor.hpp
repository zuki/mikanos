...
namespace usb {
    ...
    struct HIDDescriptor {
        ...
        /** @brief HID特有のディスクリプタに関する情報を得る。
         *
         * HIDはクラス特有(class-specific)のディスクリプタを1つ以上持つ。
         * その数はnum_descriptorsに記載されている。
         * Reportディスクリプタ(type = 34)はHIDデバイスであれば必ず存在するため、
         * num_descriptorsは必ず1以上になる。
         *
         * @param index 取得するディスクリプタの番号。 0 <= index < num_descriptors.
         * @return indexで指定されたディスクリプタの情報。indexが範囲外ならnullptr.
         */
